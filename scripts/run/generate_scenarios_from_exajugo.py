...
# =============================================================================
def _generate_load_pertubation(shape, *, noise_type="normal", var=0.1, rng=None):
    assert(noise_type == "uniform-bound")
    rng = np.random.default_rng() if rng is None else rng

    # Uniform: compute half-width to achieve desired variance
    # Var(U[-a,a]) = a^2/3 = var  =>  a = sqrt(3*var)
    half = np.sqrt(3 * var)
    print(f"use uniform(-{half}, {half})")        
    return rng.uniform(-half, half, size=shape)

def _generate_load_pertubation_uniformbound(base_p, base_q,
                           *,
                           noise_type="normal", var=0.1,
                           rng=None, return_noise=False,
                           preserve_power_factor=True,
                           basemva=100.0):
    if not (noise_type == "uniform-bound"):
        raise ValueError(f"Wrong noise_type '{noise_type}'")
    rng = np.random.default_rng() if rng is None else rng
    half = base_p * np.sqrt(3 * var)
    ub = np.minimum(half,5.0/basemva)
    lb = np.maximum(-half,-5.0/basemva)
    print(f"use uniform({lb.min()}, {ub.max()}) with elementwise bounds")
    
    p_delta = rng.uniform(lb, ub, size=base_p.shape)
    p_noise = p_delta/base_p
    ...
    if preserve_power_factor:
        ...
        q_noise = _draw_noise(base_q.shape, noise_type="uniform", var=var, rng=rng)
...
def parse_exajugo_solution(filename, basemva=100):
    ...
    for line in lines:
        ...
        if current_section == "load":
            parts = list(map(float, line.split(",")))
            load_data["i"].append(int(parts[0]))  # Bus index as integer
            load_data["p"].append(parts[1])  # Real load
            load_data["q"].append(parts[2])  # Reactive load
    ...
    for key in load_data:
        load_data[key] = np.array(load_data[key])
    for key in generator_data:
        generator_data[key] = np.array(generator_data[key], dtype=object if key == "id" else float)
    
    # only export useful data
    solution_data.g_pg = generator_data['p'].astype(float) / basemva
    solution_data.g_qg = generator_data['q'].astype(float) / basemva
    solution_data.l_pl = load_data['p'].astype(float) / basemva 
    solution_data.l_ql = -load_data['q'].astype(float) / basemva

    solution_data.b_i = bus_data['i'].astype(int)
    solution_data.b_bcs =  bus_data['bcs'].astype(float)
    solution_data.b_v =  bus_data['v'].astype(float)
    solution_data.b_theta =  bus_data['theta'].astype(float)
    
    return solution_data

def apply_exajugo_solution(psys, solution_data):
    assert isinstance(psys, Psystem)

    apply_fix = True
    if apply_fix:
        psys.set_load_pq(solution_data.l_pl,solution_data.l_ql)
        psys.set_gen_pq(solution_data.g_pg,solution_data.g_qg)
        print(solution_data.g_pg)

        # Apply ExaJuGO shunt compensation 
        bcs_by_bus = dict(
            zip(
                solution_data.b_i,
                solution_data.b_bcs,
        ...
        # Current total bsh (pu) by internal bus
        bsh_total = np.zeros(psys.nbuses, dtype=float)
        for sh in psys.shunts:
            bsh_total[sh.bus] += sh.bsh

        adjusted = 0
        for ext_bus, bcs_mvar in bcs_by_bus.items():
            if abs(bcs_mvar) <= bcs_eps:
                continue
            bus_int = psys.ext2int[int(ext_bus)]
            desired_bsh = bcs_mvar / psys.basemva
            delta_bsh = desired_bsh - bsh_total[bus_int]
            if abs(delta_bsh) <= bcs_eps:
                continue
            psys.add_shunt(bus_int, 0.0, delta_bsh * psys.basemva)
            bsh_total[bus_int] += delta_bsh
            adjusted += 1

        # Apply ExaJuGO bus voltages (Vm/theta) as PF targets for PV+slack buses
        for ext_bus, vm, theta_deg in zip(
            solution_data.b_i,
            solution_data.b_v,
            solution_data.b_theta,
        ):
            bus_int = psys.ext2int[int(ext_bus)]
            psys.buses[bus_int].v0m = float(vm)
            psys.buses[bus_int].v0a = np.deg2rad(float(theta_deg))
        return psys
...
def generate_perturbations(base_p, base_q,
    ...
    **Independent Perturbation** (``preserve_power_factor=False``):
    ...
    p_scaled = base_p * (1.0 + p_noise)
    # # Enforce absolute perturbation limit ±5.0 MW
    # p_scaled = np.clip(p_scaled, base_p - 5.0/basemva, base_p + 5.0/basemva)
...
def run_single_scenario_worker(
    ...
    solution_data: SolutionData object, optional
        A wildcard function that allow user providing some solution data to replace the  initial point.
    ...
    **Random Number Generation**:
    ...
    try:
        ...
        if perturb_loads and base_p_load.size:
            print(f"Generate pL via {load_noise_type} with var={load_noise_var}")
            if load_noise_type == "uniform-bound":
                pL_scaled, qL_scaled, pL_noise, qL_noise = _generate_load_pertubation_uniformbound(
        ...
        if gen_noise_var == 999 and perturb_gens and base_p_gen.size \
           and pg_lb is not None and pg_ub is not None:
            # -----------------------------------------------------------------
            # Step 2a+3a: Perturb generator active power, following
            #             uniform(low=pg_lb, high=pg_ub)
            # -----------------------------------------------------------------
            print("Generate pg via uniform(low=pg_lb, high=pg_ub)")   
            pG_scaled = rng_gen.uniform(low=pg_lb, high=pg_ub)
        else:
            # -----------------------------------------------------------------
            # Step 2b: Perturb generator active power
            # -----------------------------------------------------------------
            if perturb_gens and base_p_gen.size:
                print(f"Generate pG via {gen_noise_type} with var={gen_noise_var}")
            ...
            # -----------------------------------------------------------------
            # Step 3b: Clamp generator active power to limits
            # -----------------------------------------------------------------
        ...
        # -----------------------------------------------------------------
        # Step 8: use wildcard solution file
        # -----------------------------------------------------------------
        if solution_data:
            psys = apply_exajugo_solution(psys, solution_data)
        ...
        # -----------------------------------------------------------------
        # Step 8.5: use wildcard solution file, run pf (optional)
        # -----------------------------------------------------------------
        if solution_data:
            runpf(psys, verbose=True)

        pG_scaled, qG_scaled = psys.get_gen_pq()
        pL_scaled, qL_scaled = psys.get_load_pq()
...
def main(config_path: str = None):
    ...
    exajugo_sol_path : str, optional
        Path to the exajugo solution file.
...
Examples:
  ...
  python generate_scenarios.py config.json --continue --additional-samples 10
    ...
    parser.add_argument(
        ...
        nargs="?",
        default=None,
        help="Optional path to exajugo solution file"
    )
    parser.add_argument(
        "--generate-configs",
        action="store_true",
    ...
    # Load configuration
    solution_data = None
    if args.exajugo_sol is not None:
        print(f"Load exajugo solution file: {args.exajugo_sol}")
        solution_data = parse_exajugo_solution(args.exajugo_sol)
