...
def _sexs_jacobian_fixture():
    ...
    coords = exc.preallocate_jacobian(jac_idxs, None, power_injection=True)
    rows = []
    cols = []
    ...
    J = csr_matrix((data, (rows, cols)), shape=(3, 3), dtype=np.float64)
    theta = np.array([exc.TA_TB, exc.TB, exc.K, exc.TE, exc.Emin, exc.Emax, exc.vref], dtype=np.float64)
...
def test_sexs_ignores_output_limits_in_residual():
    ...
    exc.residual_diff(F, z, v, theta, residual_idxs, True)
...
def test_sexs_efd_row_matches_unconstrained_finite_difference_outside_limits():
    ...
    exc.residual_jac(J, z, v, theta, jac_idxs, power_injection=True)
    analytical = J.toarray()[1, [0, 1, 2]]
    ...
    for col in [0, 1, 2]:
        ...
        exc.residual_diff(f_p, z_p, v_p, theta, residual_idxs, True)
        exc.residual_diff(f_m, z_m, v_m, theta, residual_idxs, True)
